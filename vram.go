...
// root or as the same user as the GPU-using processes. A systemd service
// running as a different user will see only its own empty FD set and return
// (0, false) unless the process has CAP_SYS_PTRACE.
...
// AMDGPU_INFO_MEMORY ioctl to read current VRAM heap usage. Not on the default
// call path (fdinfo is preferred) — kept for diagnostic use.
...
// Keep queryAMDGPUVRAMUsed referenced so unusedfunc doesn't flag it.
var _ = queryAMDGPUVRAMUsed
...
//
// The AMD path uses fdinfo (the method nvtop uses) rather than the DRM
// AMDGPU_INFO_MEMORY ioctl: the ioctl only sees small baseline heap entries
// and misses ROCm/KFD allocations entirely. fdinfo requires running as the
// same UID as the GPU consumer (Linux denies /proc/<pid>/fdinfo reads across
// UIDs without CAP_SYS_PTRACE) — if w84ggufman runs as root or the same user
// as llama.cpp, both the targeted and full-scan paths succeed.
